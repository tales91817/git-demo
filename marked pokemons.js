        ...
        pokeCard += `
            <div class="pokemon-card">
                <div class="top">
                    ...
                    <div><ion-icon name="bookmark" class="mark-icon" data-id="${pokemon[i].id}"></ion-icon></div>
                </div>
...
pokemonMenu.addEventListener('click', function onClikckedPokemon(event) {
    ...
    if (target.matches('#moreBtn')) {
        ...
        removeFromFavoriteList(Number(target.dataset.id))
    }
...
searchForm.addEventListener('submit', function onSearchFormSubmitted(event) {
    ...
    searchedPokemon = pokemons.filter((pokemon) => 
